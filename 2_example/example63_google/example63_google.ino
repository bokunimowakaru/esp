/*******************************************************************************
Example 63 Google カレンダー(予定表) から予定を取得する

クラウド（Google Apps Script）で実行したスクリプトを本機で受け取ります。
予め本フォルダ内のGoogleAppsScript.txtを https://script.google.com/ へ転送し、
...
を転送し、関数doGet()をクラウド上で実行してください。
実行すると、Google カレンダーへのアクセスの承認が求められるので、許可します。
[公開]メニューから[ウェブアプリケーションとして導入]を選択し、「次のユーザとして
アプリケーションを実行」の欄で、公開したいカレンダーのアカウントを選択し、「アプ
リケーションにアクセスできるユーザー」で「全員（匿名ユーザーを含む）」を選択して
ください。
保存すると、下記のような公開URLが表示されます。
