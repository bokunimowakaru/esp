/*******************************************************************************
Arduino用 I2C 温湿度気圧センサ Bosch BME280 raspi_bme280
...
int _bme280_getReg(byte reg){
	Wire.beginTransmission(I2C_bme280);
	Wire.write(reg);
	if( Wire.endTransmission()==0){
		delay(1);
		Wire.requestFrom(I2C_bme280,0x1);
...
void _bme280_cal(){
	...
	dig_P7 = (s16)(_bme280_getReg(0x9A) + (_bme280_getReg(0x9E)<<8));
...
int bme280_stop(){
    ...
    reg= 0xF4;                 	// ctrl_meas
    data=0x00;
    _bme280_setByte(reg,data);  // 書込みの実行
//  i2c_close();
}
