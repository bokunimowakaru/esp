...
#define DEBUG_MAC 16						// MAC情報表示の表示間隔(攻撃防止)

MAC_header _mac_prev;
int _mac_prev_n=0;
...
static void ICACHE_FLASH_ATTR promisc_cb(uint8_t *buf, uint16_t len){
	...
	if(memcmp(&_mac_prev,mac,22)==0){
		_mac_prev_n++;
		if( (n%DEBUG_MAC) !=(DEBUG_MAC-1) ) return;
	}
	memcpy(&_mac_prev,mac,sizeof(_mac_prev));
	n=_mac_prev_n;
	_mac_prev_n=0;
