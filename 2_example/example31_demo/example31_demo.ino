/*******************************************************************************
Example 31:最新IoTモジュール ESP32-WROOM-32用 ESP32 DEMO (HTTP版) デモ用

展示会などでPCやスマホから直接ESPモジュールへ接続する場合のサンプルです。
ESP32内に内蔵されているホール効果素子センサと、タッチセンサの検出値をスマホの
ウェブブラウザへ表示します。

解説
　Wi-Fiモジュール使ったデモでは、ネットワークを組んだり、PC やスマホを接続したり
　する際の手間が課題となることがあります。
　そこで、ESP32に内蔵されている無線LAN アクセスポイント機能を使用し、一時的な
　ネットワーク環境下でESP32に内蔵されているホール効果素子センサとタッチセンサの
　読み値を確認できるサンプルを作成しました。

ESP32内蔵 ホール効果素子センサ
　ホール効果素子センサは磁界を検出することができます。
　磁石をESP32 モジュールへ接触させたり遠ざけたりするとプラスまたはマイナス方向へ
　読み値が変化します．

ESP32内蔵 タッチセンサ
　タッチセンサ用に使用可能なピンはIO 0，2，4，12～15，27，32 ～ 33 の10 ポート
　です。これらのポートを指で触れると、センサ状態（ONまたはOFF）が変化します。

スマホでセンサ値を確認する
　スマホからESP32の無線LANへ接続し、ウェブブラウザでホール効果素子センサとタッチ
　センサの値を確認することが出来ます。
　スマホ側のIPアドレスはESPモジュール側のDHCPサーバが自動的に割り振りふります。
　ESPモジュール側のIPアドレスは固定（192.168.1.2）です。
　二次元バーコードなどから簡単にアクセスできるでしょう。
　
