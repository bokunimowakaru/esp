...
#define PIN_POW 2                           // GPIO 2(24番ピン)に電源制御回路
#define TIMEOUT 20000                           // タイムアウト 20秒
#define SSID "1234ABCD"                         // 無線LANアクセスポイントのSSID
#define PASS "password"                         // パスワード
#define PiUSER "pi"                             // Raspberry Pi ユーザ名

HardwareSerial hardSerial(2);               // Raspberry Pi接続用シリアルポートESP32用
...
void loop(){                                    // 繰り返し実行する関数
    ...
    while(hardSerial.available()) hardSerial.read();   // 受信バッファのクリア
    ...
    if(com[0]){                                 // コマンドあり
        ...
        while(hardSerial.available())hardSerial.read(); // 受信バッファのクリア
        rx[0]='\0';                             // 受信データのクリア
        hardSerial.print(com);                  // 文字データを出力
        hardSerial.print('\n');                 // 改行コードLFを出力
    }
    while(rxi<1024){                            // シリアル受信ループ(1024文字)
        for(t=0;t<1000000;t++){                 // 約300msの期間、受信確認
            if(hardSerial.available()) break;   // 受信があれば確認完了
        }
        if(t==1000000)break;                    // 受信が無ければ受信完了
        c=hardSerial.read();                    // シリアルから受信
        if(c=='\n'){                            // 改行時
