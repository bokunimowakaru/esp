...
static uint8 gPicoJpegCallbackStatus=0;
...
static void fillInBuf(void)
{
   ...
   if (status)
   {
      ...
      gPicoJpegCallbackStatus = status;
   }
}   
//------------------------------------------------------------------------------
...
static uint8 findEOI(void)
{
   ...
   if (status)
      return status;
   else if (gPicoJpegCallbackStatus)
      return gPicoJpegCallbackStatus;
...
unsigned char pjpeg_decode_mcu(void)
{
   ...
   */
   ...
   if ((status) || (gPicoJpegCallbackStatus))
      return gPicoJpegCallbackStatus ? gPicoJpegCallbackStatus : status;
   */
