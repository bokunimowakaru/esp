/*******************************************************************************
Example 25a: センサ受信データ・ファイルシステム [FTPサーバ機能付き]
                                            Copyright (c) 2016 Wataru KUNINO
*******************************************************************************/

// ご注意：FTPサーバは簡易的なものです。
// FTPクライアント側には FileZilla を使用してください。
// FFFTP でも、おおむね使用可能なことを確認しています。
// Windows エクスプローラのFTPクライアントでは動作確認ができていません。
// FTPサーバに関わらず、すべての実装が実験目的のものです。補償は一切ありません。
...
void loop() {
    ...
    if(client==0){                          // TCPクライアントが無かった場合
        ...
        Serial.println(date);               // 日時のシリアル出力表示
        Serial.println(s);                  // デバイス名のシリアル出力表示
        Serial.println(&s[8]);              // データのシリアル出力表示
        lcdPrint(s);                        // 液晶に表示する
    ...
    if(strncmp(s,"GET /?FORMAT",12)==0){    // ファイルシステム初期化コマンド
        Serial.println("FORMAT");           // シリアル出力表示
