...
TFTPはネットワーク機器などの設定ファイルやファームウェアを転送するときに使用されるデータ転送プロトコルです。  
使い勝手が簡単で、プロトコルも簡単なので、機器のメンテナンスに向いています。通常、認証や暗号化といったことは行わず、転送時のみ有効にする、もしくは変更されても問題の無い用途で利用します。

## 本サンプルの仕様

TFTPで「ADC_PIN=ピン番号」を送信すると、ESPモジュールのADC入力ピンを変更することが出来ます。また、「SLEEP_SEC=時間（秒）」を送信すると、ESPモジュールのスリープ間隔を変更することが出来ます。
